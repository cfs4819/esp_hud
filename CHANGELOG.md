...
### 变更
- Java SDK 轨迹简化策略升级为在线增量简化，在持续定位输入场景下保持固定点数预算，避免轨迹点单向增长导致的性能波动。
- 地图请求链路改为直接使用在线简化结果，提升长时间运行时的刷新稳定性与响应一致性。
- Java SDK 补充显示翻转枚举的公开协议值访问能力。
...
### 新增
- 增加显示控制能力，支持运行时调节亮度与显示方向。
- 上位机示例增加对应控制能力，便于联调与验证。

### 变更
- Java SDK 地图刷新策略优化，补充首帧与周期刷新机制，提升地图显示稳定性。
- Java SDK 地图链路可观测性增强，便于定位拉图、入队与发送链路问题。
- README 同步更新使用说明与示例内容。
